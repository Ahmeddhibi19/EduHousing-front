...
const AddApartment = () => {
  ...
  useEffect(() => {
    console.log(apartmentData.images);
  }, [apartmentData.images]);
 const userId =localStorage.getItem("userId");
  const handleSubmit = (e) => {
    e.preventDefault();
    if(!apartmentData.latitude || !apartmentData.longitude || !apartmentData.address || !apartmentData.type || !apartmentData.images){
    ...
    axios.post(`http://localhost:8081/api/EduHousing/v1.0.0/apartment/homeowner/create/${userId}/${cityId}`, apartment, {
  headers: {
    Authorization: `Bearer ${accessToken}`
  }
})
    ...
    .then(res=>{
      ...
      ).then((response)=>{}).catch((error)=>{console.log(error);setResponseData(error)});
      });
  ...
  return (
    <div className='w-full sm:w-full md:w-full lg:w-[80%] h-auto sm:ml-2 lg:ml-[20%] mt-[63px] px-3' onWheel={handleScroll}>
      ...
      <FontAwesomeIcon icon={faExclamationCircle} className='mt-1 mr-1'/>
        <h1 className='text-[18px] font-bold '>Please make sure to select your apartment location on the map meticulously, as the system relies on accurate coordinates for extraction.
        </h1>
      </div>
...
*/}
      ...
      <h1 className='text-[18px] my-5 font-bold  text-gray-600'>choose the apartment City :</h1>
      <select
  value={cityId}
